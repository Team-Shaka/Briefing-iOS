...
class BriefingCardViewController: UIViewController {
    private let briefingUseCase = AppDIContainer.shared.briefingUseCase
    private let scrapUseCase = AppDIContainer.shared.scrapUseCase
...
//        
    ...
    private func fetchBriefingCard() {
        ...
        briefingUseCase.fetchBriefingCard(id: id)
    ...
    private func scrapBriefingCard() {
        let scrapStream = isScrap ? scrapUseCase.deleteScrapBriefing(id: id) : scrapUseCase.scrapBriefing(id: id)
        scrapStream
    ...
    private func scrapResultCompletion(_ result: ScrapResult?, _ error: Error? = nil) {
        ...
        guard let isScrap = result?.isScrapped else { return }
