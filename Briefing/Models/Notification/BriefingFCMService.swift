...
final class BriefingFCMService: NSObject, BFNetworkManager {
...
private extension BriefingFCMService {
    func subscribe(_ fcmToken: String?) {
        ...
        guard let urlRequest = BriefingFCMURLRequest(member?.accessToken,
                                               url: url,
                                               method: .post,
                                               path: .subscribe,
                                                     httpBody: [.token: fcmToken]) else { return }
